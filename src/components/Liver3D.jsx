...
const ObjFilesRenderer = (props) => {
  ...
  const [viewAllNames, setViewAllNames] = useState(false);
  ...
  const handdleRestore = () => {
    ...
    setActors((prevActors) => {
      ...
      updatedActors.forEach((actorObj, index) => {
        ...
        vtkActor.setOrientation(270, 0, 0);
  ...
  const customOrder = ['arteries', 'veins', 'lesion', 'seg_1', 'seg_2', 'seg_3', 'seg_4', 'seg_5', 'seg_6', 'seg_7', 'seg_8', 'airway']; // Define your desired order

  const getOrderIndex = (name) => {
    const index = customOrder.findIndex((item) => name.toLowerCase().includes(item));
    return index === -1 ? customOrder.length : index; // Assign a large index if not found
  };

  // Create a sorted array while keeping the original index intact
  const sortedActors = [...actors]
    .map((actorObj, originalIndex) => ({ ...actorObj, originalIndex })) // Add the original index
    ...
    <div className="container" style={{ position: 'relative' }} ref={pageContainerRef}>
      ...
      <div style={{ position: 'absolute', top: '15px', right: '5px', zIndex: 1 }}>
        ...
        {isRotating ? (
          <Tooltip title="Stop Rotation">
            ...
            >
              <SyncDisabledOutlinedIcon sx={{ color: "#fff" }} />
            </IconButton>
          </Tooltip>
        ) : (
          <Tooltip title="Start Rotation">
            ...
            >
              <AutorenewOutlinedIcon sx={{ color: "#fff" }} />
            </IconButton>
          </Tooltip>
        )}
      ...
      {rendererInitialized && isControlsVisible && (
        <div className="controls">
          {sortedActors.map((actorObj) => (
            actorObj.name.toLowerCase().includes('mtl') && !viewAllNames ? null : // Hide mtl objects 
              <div key={actorObj.originalIndex} style={{ color: actorObj.color }} className="control-item">
                <Button onClick={() => toggleVisibility(actorObj.originalIndex)}> {/* Use original index */}
                  {actorObj.visibilityMode === 'visible' && <VisibilityIcon style={{ color: actorObj.color }} />}
                  {actorObj.visibilityMode === 'transparent' && <VisibilityTransparentIcon style={{ color: actorObj.color }} />}
                  {actorObj.visibilityMode === 'invisible' && <VisibilityOffIcon style={{ color: actorObj.color }} />}
                </Button>
                {actorObj.name}
                <input
                  ...
                  onChange={(e) => handleColorChange(actorObj.originalIndex, e.target.value)} // Use original index
                  style={{
      ...
      {rendererInitialized && isControlsVisible && (
        <div style={{ position: 'absolute', top: '10px', left: '70px', zIndex: 1 }}>
          <Tooltip title="Show more Airways segments">
            <IconButton
              id="show_more_btn"
              onClick={() => {
                setViewAllNames(!viewAllNames);
              }}
            >
              <ExpandMoreIcon sx={{ color: viewAllNames ? '#EB3678' : "white" }} />
            </IconButton>
          </Tooltip>
        </div>
      )}
